            ...
            <tr v-for="(data,index) in lembur.data" :key="data.id">
              ...
              <td class="align-middle text-center text-sm" >
                <span v-if="data.status_lembur == 'Diproses'">
                  <span style="margin-right:7px;cursor:pointer" class="badge badge-sm bg-warning" @click.prevent="approvelembur(data.id)">approvement</span>
                  <span style="cursor:pointer" class="badge badge-sm bg-primary" @click.prevent="detailLembur(data.id)">detail</span>
                </span>
